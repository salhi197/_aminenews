...
class AnWP_Post_Grid_Elements {
	...
	public function promote_premium_widgets( $config ) {
		...
		if ( class_exists( 'AnWP_Post_Grid_Premium' ) ) {
			return $config;
		}
	 ...
	 * @return array
	 */
	public function get_grid_posts( $options ) {
		...
		*/
		...
		if ( 'custom' === $options->posts_to_show ) {
			...
			// Limit
			$args['numberposts'] = intval( $options->limit );
	...
	public function render_post_category_link_filled( $term_obj, $class = '' ) {
		...
		echo '<div class="anwp-pg-category__wrapper-filled px-2 py-1 d-flex align-items-center ' . esc_attr( $class ) . '" style="background-color: ' . esc_attr( $category_color ) . '">';
